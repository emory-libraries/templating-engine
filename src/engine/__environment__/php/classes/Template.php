...
trait Template_Utilities {
  ...
  private function __getTemplatePath( $id ) {
    ...
    return CONFIG['handlebars']['templates']."/{$id}".CONFIG['ext']['template'];
  ...
  private function __getTemplateFileMeta( $id ) {
    ...
    $default = CONFIG['handlebars']['templates']."/{$id}".CONFIG['ext']['template'];
    $cached = CONFIG['cache']['templates']."/{$id}".CONFIG['ext']['cache'];
...
trait Template_Cache {
  ...
  private function __initCache() {
    ...
    // Initialize the cache.
    $this->cache = new Cache(CONFIG['cache']['root']);

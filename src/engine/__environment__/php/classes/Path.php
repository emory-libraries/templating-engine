...
class Path {
  ...
  public static function init() {
    ...
    if( !isset(self::$domainRoot) ) {
      ...
      else if( defined('DEVELOPMENT') and DEVELOPMENT ) {

        // Capture local development environments.
        if( $_SERVER['REMOTE_ADDR'] == '127.0.0.1' ) self::$domainRoot = 'localhost/templating-engine/public/'.DOMAIN;

        // Otherwise, capture remote development environments.
  ...
  public static function toRootRelative( string $path ) {
    ...
    if( isset($parsed['scheme']) ) {
      ...
      if( $parsed['host'] != static::$domainRoot ) return $path;
    ...
    return static::$pathRoot.'/'.ltrim($path, '/');
  ...
  public static function toRelative( string $path ) {
    ...
    return ltrim(ltrim_substr($path, static::$pathRoot), '/');
  ...
  public static function toAbsolute( string $path ) {
    ...
    return $protocol.static::$domainRoot.'/'.ltrim($path, '/');

 ...
 * Pattern
 *
 * Reads and parses a pattern file given a file path.
 */
class Pattern {
  ...
  // The contents of the pattern file.
  public $pattern = '';
  
  // Initializes if the pattern is a template.
  public $template = false;
  
  // The associated page type for the template, if applicable.
  public $pageType;
  ...
  function __construct( $path ) {
    ...
    // Extract pattern data when given a valid path.
    if( is_string($path) ) {
    
      // Save the pattern file.
      $this->file = $path;

      // Read the pattern file at the given path.
      $this->pattern = File::read($path);

      // Get the pattern file's ID, name, plid, and include path.
      $this->id = self::id($path);
      $this->name = self::name($path);
      $this->plid = self::plid($path);
      $this->path = self::path($path);
      
      // Determine if the pattern is a template.
      if( self::parse($path)['group']['name'] == 'templates' ) {
        
        // Set the template flag.
        $this->template = true;
        
        // Get the template's associated page type.
